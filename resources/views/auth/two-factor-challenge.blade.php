...
@section('subheading', 'Enter your one-time sign-in code')
...
@section('content')
    <p class="flow-copy">
        We sent a <strong>6-digit code</strong> to <strong>{{ $maskedEmail }}</strong>. Enter it to complete sign-in. The code expires in 10 minutes.
    </p>
    ...
    <form method="post" action="{{ route('publisher.login.2fa.verify') }}">
        @csrf
        @if ($errors->any())
        ...
        <button class="submit" type="submit">Verify and continue</button>
    </form>
